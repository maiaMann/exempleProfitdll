...
int loadDll(const char* name)
{
    ...
    g_GetAllTicker = reinterpret_cast<GetAllTicker>(GetProcAddress(hdl, "GetAllTicker"));
    ...
    if (!g_initialize || !g_initializeMarket || !g_finalize || !g_subscribe
        ...
        || !g_SendCancelAllOrders || !g_GetPosition || !g_SetAssetList || !g_GetAllTicker
        ...
        if (bMarketConnected && bAtivo)
        {
            if (!isSubscribed)
            {
                ...
                */
                ...
                // Descomentar para solicitar informações sobre os ativos
                /*
                if (g_GetAllTicker(bolsa) != NL_OK)
                {
                    printf("Nao foi possivel retornar informacoes dos tickers\n");
                }
                */
