...
type
  ...
  public
    ...
    procedure UpdatePriceBook(                                rAssetID : TAssetIDRec ;
                              nAction , nPosition, Side, nQtds, nCount : Integer;
                                                                sPrice : Double;
                                                 pArraySell, pArrayBuy : Pointer);
    procedure UpdateOfferBook(                                            rAssetID : TAssetIDRec ;
...
const
{$IFDEF CPUX64}
  C_CURRENTVER = '4.0.0.1';
{$ELSE}
  C_CURRENTVER = '3.0.0.16';
{$ENDIF}
...
begin
  ...
  try
    ...
    begin
      ...
      AddObject('GetAllTicker',                 TObject(tMD));
...
begin
  ...
  begin
    ...
    if sText = 'SubscribeTicker' then
      ...
      lRes:= SendBuyOrder(PWideChar(edtBrokerAccRot.Text),PWideChar(edtBrokerIdRot.Text),
      ...
      lRes:= SendSellOrder(PWideChar(edtBrokerAccRot.Text),PWideChar(edtBrokerIdRot.Text),
                   PWideChar(edtPassRot.Text), PWideChar(edtTickerRot.Text),
                   PWideChar(cbBolsaRot.Items.Strings[cbBolsaRot.ItemIndex]),
                   StrToFloat(edtPriceRot.Text), StrToInt(edtAmountRot.Text))
    else if sText = 'SendStopBuyOrder' then
      lRes:= SendStopBuyOrder(PWideChar(edtBrokerAccRot.Text),PWideChar(edtBrokerIdRot.Text),
                       PWideChar(edtPassRot.Text), PWideChar(edtTickerRot.Text),
                       PWideChar(cbBolsaRot.Items.Strings[cbBolsaRot.ItemIndex]),
      ...
      lRes:= SendStopSellOrder(PWideChar(edtBrokerAccRot.Text),PWideChar(edtBrokerIdRot.Text),
                       PWideChar(edtPassRot.Text), PWideChar(edtTickerRot.Text),
                       PWideChar(cbBolsaRot.Items.Strings[cbBolsaRot.ItemIndex]),
                       StrToFloat(edtPriceRot.Text), StrToFloat(edtPriceStopRot.Text),
                       StrToInt(edtAmountRot.Text))
      ...
      lRes:= SendZeroPosition(PWideChar(edtBrokerAccRot.Text), PWideChar(edtBrokerIdRot.Text),
      ...
      lRes:= SendMarketSellOrder(PWideChar(edtBrokerAccRot.Text), PWideChar(edtBrokerIdRot.Text),
                   PWideChar(edtPassRot.Text), PWideChar(edtTickerRot.Text),
                   PWideChar(cbBolsaRot.Items.Strings[cbBolsaRot.ItemIndex]), StrToInt(edtAmountRot.Text))
    else if sText = 'SendMarketBuyOrder' then
      lRes:= SendMarketBuyOrder(PWideChar(edtBrokerAccRot.Text), PWideChar(edtBrokerIdRot.Text),
                   PWideChar(edtPassRot.Text), PWideChar(edtTickerRot.Text),
                   PWideChar(cbBolsaRot.Items.Strings[cbBolsaRot.ItemIndex]), StrToInt(edtAmountRot.Text))
    ...
    else if sText = 'GetAllTicker' then
      nRes:= GetAllTicker(PWideChar(cbBolsaMd.Items.Strings[cbBolsaMd.ItemIndex]))
    ...
    if nRes > -1 then
      mmFunc.Lines.Add(Format(sText + ': %d', [nRes]))
    else if lRes > -1 then
      mmFunc.Lines.Add(Format(sText + ': %d', [lRes]))
...
begin
  ...
  TryFloatToDateTime(lDate, lData);
...
begin
  ...
  begin
    ...
    if pArraySell <> nil then
      DecryptPriceArray(pArraySell, ListSellPriceBook);
    if pArrayBuy <> nil then
      DecryptPriceArray(pArrayBuy, ListBuyPriceBook);
...
begin
  with frmClient do
    begin
      if  pArraySell <> nil
        then DecryptOfferArray(pArraySell, ListSellOfferBook);
      if  pArrayBuy <> nil
        then DecryptOfferArray (pArrayBuy, ListBuyOfferBook);
      ...
      if (nPosition = StrToInt(edtBookPos.Text)) then
        begin
          Group  := lBook.Items[lBook.Count - 1 - nPosition];
